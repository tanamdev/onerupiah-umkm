...
export default function AdminPackagesPage() {
  ...
  const openEdit = (pkg: PackageData) => {
    ...
    setForm({
      ...
      maxContentGenerations: pkg.maxContentGenerations
        ? String(pkg.maxContentGenerations)
        : '',
      maxImageGenerations: pkg.maxImageGenerations
        ? String(pkg.maxImageGenerations)
        : '',
      imagesPerGeneration: pkg.imagesPerGeneration
        ? String(pkg.imagesPerGeneration)
        : '',
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const payload = {
        ...
        maxContentGenerations: form.maxContentGenerations
          ? Number(form.maxContentGenerations)
          : null,
        maxImageGenerations: form.maxImageGenerations
          ? Number(form.maxImageGenerations)
          : null,
        imagesPerGeneration: form.imagesPerGeneration
          ? Number(form.imagesPerGeneration)
          : null,
      ...
      <div className="flex justify-between items-center bg-white/50 p-4 rounded-xl border border-gray-100 backdrop-blur-sm">
        <div>
          ...
          <p className="text-gray-500 mt-1">
            Kelola paket langganan yang tersedia untuk pengguna.
          </p>
        </div>
        <button
          onClick={openCreate}
          ...
          {packages.length === 0 ? (
            ...
            packages.map((pkg) => {
              ...
              return (
                <Card
                  key={pkg.id}
                  className="border-gray-200 shadow-sm hover:shadow-md transition-shadow flex flex-col relative overflow-hidden"
                >
                  {/* Status bar */}
                  <div
                    className={`h-1.5 w-full ${pkg.isActive ? 'bg-gradient-to-r from-blue-400 to-indigo-500' : 'bg-gray-200'}`}
                  />
                    ...
                    <div className="mt-3 space-y-0.5">
                      ...
                      {pkg.yearlyPrice && (
                        <div className="text-sm text-gray-500">
                          Tahunan: {pkg.currency}{' '}
                          {pkg.yearlyPrice.toLocaleString('id-ID')}
                        </div>
                      )}
                    </div>
                  </CardHeader>
                    ...
                    <div className="mb-3 text-sm text-gray-600 space-y-1">
                      <div>
                        ...
                        <span className="font-medium text-gray-800">
                          {pkg.maxContentGenerations ?? 'Unlimited'}
                        </span>
                      </div>
                      <div>
                        Gambar:{' '}
                        <span className="font-medium text-gray-800">
                          {pkg.maxImageGenerations ?? 'Unlimited'} (
                          {pkg.imagesPerGeneration ?? 2}/click)
                        </span>
                      </div>
                    </div>
                    ...
                    {featureList.length > 0 && (
                      <ul className="text-sm text-gray-600 space-y-1 mb-4">
                        ...
                        {featureList.length > 4 && (
                          <li className="text-xs text-blue-600 italic pl-5">
                            +{featureList.length - 4} fitur lainnya
                          </li>
                        )}
                      </ul>
                    )}
          ...
          <div className="bg-white rounded-2xl shadow-2xl w-full max-w-xl max-h-[90vh] overflow-y-auto">
            ...
            <form onSubmit={handleSubmit} className="px-6 py-5 space-y-4">
              ...
              <div className="grid grid-cols-2 gap-3">
                <div>
                  <label className="block text-sm font-medium text-gray-700 mb-1">
                    Harga Bulanan (IDR) <span className="text-red-500">*</span>
                  </label>
                  <input
                    type="number"
                    required
                ...
                <div>
                  ...
                  <input
                    ...
                    onChange={(e) =>
                      setForm((p) => ({
                        ...p,
                        maxContentGenerations: e.target.value,
                      }))
                    }
                ...
                <div>
                  ...
                  <input
                    ...
                    onChange={(e) =>
                      setForm((p) => ({
                        ...p,
                        maxImageGenerations: e.target.value,
                      }))
                    }
                    placeholder="Unlimited"
                    className="w-full h-10 px-3 border border-gray-200 rounded-lg text-sm text-gray-900 placeholder:text-gray-400 focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent"
              ...
              <div className="grid grid-cols-2 gap-4 mt-4">
                <div>
                  <label className="block text-sm font-medium text-gray-700 mb-1">
                    Gambar Per 1x Generate
                  </label>
                  <input
                    type="number"
                    min={1}
                    max={4}
                    value={form.imagesPerGeneration}
                    onChange={(e) =>
                      setForm((p) => ({
                        ...p,
                        imagesPerGeneration: e.target.value,
                      }))
                    }
          ...
          <div className="bg-white rounded-2xl shadow-2xl w-full max-w-sm p-6">
            <div className="flex flex-col items-center text-center gap-3">
              ...
              <p className="text-sm text-gray-500">
                ...
                .
                {(deleteTarget.totalSubscriptions > 0 ||
                  deleteTarget.totalCompletedTransactions > 0) && (
                  <span className="block mt-1 text-amber-600 font-medium">
                    Paket ini masih memiliki data terkait dan akan
