...
export async function GET(request: NextRequest) {
  try {
    ...
    if (!reference && !merchantOrderId) {
      ...
      try {
        ...
        if (user) {
          ...
          const recentTransactions = await BillingService.getUserTransactions(
            user.id,
            5,
            0,
          );
          ...
          if (recentTransactions.length > 0) {
            ...
            return NextResponse.json({
              ...
              fallback: true,
              message:
                'Using recent transaction as fallback (URL parameters were missing)',
              data: {
                transactionId: latestTransaction.id,
                merchantOrderId: (latestTransaction.metadata as any)
                  ?.merchantOrderId,
      ...
      return NextResponse.json(
        {
          success: false,
          error:
            'Missing reference or merchantOrderId parameter. Please provide either reference or merchantOrderId from the payment URL.',
    ...
    if (transaction.externalId) {
      try {
        console.log(
          '🔍 Checking status with Duitku API for reference:',
          transaction.externalId,
        );
        const duitkuStatus = await duitkuService.getTransactionStatus(
          transaction.externalId,
        );
        ...
        const newStatus = duitkuService.parsePaymentStatus(
          duitkuStatus.resultCode.toString(),
        );
        ...
        const isSuccess =
          duitkuStatus.paymentStatus === 1 && duitkuStatus.resultCode === '00';
