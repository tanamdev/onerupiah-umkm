...
export async function POST(request: NextRequest) {
  try {
    ...
    const body: DuitkuCallbackData = Object.keys(formDataObj).reduce(
      (acc, key) => {
    ...
    const missingFields = requiredFields.filter(
      (field) => !body[field as keyof DuitkuCallbackData]
    );
    ...
    if (!duitkuService.validateNewCallbackSignature(body, body.signature)) {
      ...
      console.error(
        'Expected signature format: MD5(merchantcode + amount + merchantOrderId + merchantKey)'
      );
    ...
    if (isSuccess) {
      try {
        ...
        const { packageId, period, metadata } = parseTransactionMetadata(
